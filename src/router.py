...
async def _generate_fact() -> Fact:
    ...
    category = random.choice(list(FactCategory))
    ...
    try:
        message = client.messages.create(
            ...
            messages=[
                {
                    "role": "user",
                    "content": f"Generate one interesting, accurate fact about {category.value}. Just the fact, no preamble.",
                }
            ],
        )
